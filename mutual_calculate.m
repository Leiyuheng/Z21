...
for i = 1:points
    ...
    % 将符号表达式转换为函数句柄
    E_func = matlabFunction(E, 'Vars', t);

    % 使用积分函数对 E 进行数值积分
    E_integral = integral(E_func, 0, 1, 'ArrayValued', true);
    Ef(i, :) = E_integral;
end
...
f_inte_re = @(x) interp1(r_int,real(Ef_inte_disc),x,'makima');
f_inte_im = @(x) interp1(r_int,imag(Ef_inte_disc),x,'makima');
